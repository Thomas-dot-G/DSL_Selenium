...
@after {
	...
	(
		...
		(
			...
			{
				$current = $this_SimpleOp_2.current;
				afterParserOrEnumRuleCall();
			}
			    |
			{
...
// Entry rule entryRuleFindIf
entryRuleFindIf returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getFindIfRule()); }
	iv_ruleFindIf=ruleFindIf
	{ $current=$iv_ruleFindIf.current; }
	EOF;

// Rule FindIf
ruleFindIf returns [EObject current=null]
@init {
	enterRule();
}
@after {
...
RULE_HTML_ELT : ('id'|'class'|'tag'|'name');
